	...
	public partial class Form1 : Form
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			for (int j = 0; j <= dtDuno.Rows.Count - 1; j++) // chạy bảng khách hàng đã phân công theo cán bộ
			{
				...
				dunohientai = new int[5] { 0, 0, 0, 0, 0 };
				...
				for (int z = j + 1; z <= dtDuno.Rows.Count - 1; z++)
				{
					...
					if (dtDuno.Rows[j][0].ToString() == dtDuno.Rows[z][0].ToString()) //nếu trùng mã KH thì kiểm tra LAV
					{
						MessageBox.Show("trùng mã KH");
						run++;
						if (dtDuno.Rows[j][3].ToString() == dtDuno.Rows[z][3].ToString()) //nếu trùng LAV thì tính tổng dư nợ hiện tại
						{
							MessageBox.Show("trùng LAV");
							count++;
							...
							label2.Text = dtDuno.Rows[j][2].ToString() + " - LAV: " + dtDuno.Rows[j][3].ToString() + " - " + " mucdichvay0: " + mucdichvay[0] + " - giatritt[0]: " + giatritt[0]+"\n" ;
							

						}
						else //Nếu không trùng LAV thì ghi vào doc
						{
							MessageBox.Show("khác LAV");
					...
					}// không trùng mã KH thì in biên bản tài sản
					else
					{
						filename = (pa + "\\12A.docx");
						object missing = System.Type.Missing;
						...
						doc = new Document();
						filename = "";
						pa = System.Environment.CurrentDirectory;
